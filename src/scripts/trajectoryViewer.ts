...
export function initTrajectoryViewer() {
  ...
  async function loadTaskIndex() {
    try {
      ...
      const response = await fetch('/mindcube/data/trajectories/qwen2.5-0.5b/index.json');
      if (!response.ok) {
  ...
  function navigateToStep(stepIndex: number) {
    ...
    if (stateImageElement) stateImageElement.src = step.state.image || '/mindcube/placeholder-state.png';
